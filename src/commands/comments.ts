...
async function handleCommentsList(pageId: string, options: {
  format?: string; limit?: string; start?: string;
  location?: string; depth?: string; all?: boolean;
  ...
  const invalidLocations = locationValues.filter(
    (v) => !['inline', 'footer', 'resolved'].includes(v),
  );
  ...
  const renderComments = (
    nodes: Array<(typeof commentTree)[number]>,
    path: number[] = [],
  ) => {
    for (let i = 0; i < nodes.length; i++) {
      ...
      if (!isReply && comment.inlineProperties) {
        const selectionText = comment.inlineProperties.selection
          ?? comment.inlineProperties.originalSelection;
        if (selectionText) {
          const selectionLabel = comment.inlineProperties.selection
            ? 'Highlight'
            : 'Highlight (original)';
      ...
      const body = client.formatCommentBody(
        comment.body,
        format as 'storage' | 'html' | 'markdown' | 'text',
      );
      if (body) {
      ...
      if (comment.children?.length) {
        renderComments(
          comment.children as Array<(typeof commentTree)[number]>,
          currentPath,
        );
      }
    }
...
async function handleCommentCreate(pageId: string, options: {
  file?: string; content?: string; format?: string;
  parent?: string; location?: string; inlineSelection?: string;
}, analytics: Analytics): Promise<void> {
  const config = getConfig();
  ...
  if (location === 'inline' && !options.inlineSelection && !options.parent) {
    throw new Error(
      'Inline comments require --inline-selection when starting a new inline thread.',
    );
  }
  ...
  const result = await client.create(
    ...
    (options.format ?? 'storage') as 'storage' | 'html' | 'markdown' | 'text',
    {
      ...
      location,
      inlineProperties: Object.keys(inlineProperties).length > 0
        ? inlineProperties
        : undefined,
    },
  );
...
export function registerCommentCommands(program: Command): void {
  ...
  const comment = program
    .command('comment')
    .description('Comment operations (grouped)');
