...
async function handleAttachmentUpload(
  ...
  for (const file of resolvedFiles) {
    ...
    console.log(`  ${chalk.green(result.title)} (ID: ${result.id}, Version: ${result.version})`)
    uploaded++
  }
...
export function registerAttachmentCommands(program: Command): void {
  ...
  const attachment = program.command('attachment').description('Attachment operations (grouped)')
