...
export function registerBlogCommands(program: Command): void {
      ...
      async (
        ...
        try {
          ...
          if (content === undefined) {
            console.error(
              chalk.red('Error:'),
              'Provide content via --file <path> or --content <text>.',
            );
            process.exit(1);
          }
          const client = new BlogClient(new HttpClient(getConfig()));
          const post = await client.create(title, space, content, options.format as any);
  ...
  blog
    ...
    .action(
      async (
        id: string,
        options: { title?: string; file?: string; content?: string; format?: string },
      ) => {
        const analytics = new Analytics();
        try {
          const content = resolveContent(options);
          ...
          const client = new BlogClient(new HttpClient(getConfig()));
          const post = await client.update(id, content, options.format as any, options.title);
