...
export async function handleCreate(title: string, space: string, options: {
...
export async function handleUpdate(pageId: string, options: {
...
export async function handleMove(pageId: string, parent: string, options: { title?: string }, analytics: Analytics): Promise<void> {
  const config = getConfig();
  assertWritable(config);
  const { pages } = buildClient(config);
...
export async function handlePageList(space: string, options: {
